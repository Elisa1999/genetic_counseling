...
```
## Abstract
In the report, we are interested in exploring the relationship between cancer stage, ECOG level, sex assigned at birth, ethnicity, age, and genetic referral rate. We fit a logistic regression model to identify the relationship between predictors and the outcome variable. The output of the model indicates that there is insufficient statistical evidence that the correlation between each predictor and outcome variable is significant at a significance level of 0.05. 

## Introduction
The overall report consists of three parts. 
In the first part, we include visualizations of the relationship between each potential predictor and referral rate. From the visualizations, we could conclude that there is no obvious relationship between each predictor and the response variable.
