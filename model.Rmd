...
In this report, we focus on building an inferential model to evaluate the association between key variables and genetic referral rate. The predictor variables that we include in the model are cancer stage, ECOG level, sex assigned at birth, ethnicity and age.

We include visualizations of the relationship between each predictor and referral rate. From the visualizations, we see that there does not appear to be much of a marginal relationship between each predictor and the response.

The model which we use is a logistic regression model. The results match what the visualizations seem to be telling us--we do not see strong evidence of a relationship between any of the predictors and referral rate.

One potential concern in modelling is if predictor variables are highly correlated, as this can lead unreliable inferences. As ECOG measures the level of a patients functioning (in terms of daily living abilities), we felt that there may be reason to believe that it could be associated with cancer stage. Therefore, we include two additional models in the appendix: one which omits ECOG from the predictors, and one which omits stage from the predictors, to see whether there is a difference. In all models, there is no strong evidence of a relationship between the predictors and the response.


## Visulization

Since visualizing the dependent variables in the model helps us anticipate and make sense of results, we try to generate a series of plots on each variable. In each plot, we generate error bars using the formula in the appendix.
...
Since the all of p-values are greater than 0.05, we don't have enough confidence to reject the null hypothesis that the variables have no correlations with the dependent variable. Consequently, it's insufficient to infer that these variables contribute significantly to genetic referrals.
